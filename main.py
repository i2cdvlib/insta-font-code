...
image_style = f"background-image: url({image_url}); background-size: contain; background-repeat: no-repeat; height: 150px; width: 150px;"
st.markdown(f'<div style="{image_style}"></div>', unsafe_allow_html=True)

# Add text area input for phone number, email, and Instagram ID
info = st.text_area("Phone Number, Email, Instagram ID", height=100)
...
login_button = st.button(login_button_text, key="login_button", help="Click to login", style=login_button_style)
